...
function getData() {
	...
	getReports(start, end, 'internal');
	getTableReports(start, end, 'internal');
}

function getReports(startTime, endTime, type) {

	$('#chart_drop_pps').empty();
	...
	$.ajax({
		...
		success: function(data){
			if(data.length==0){
				$('#chart_drop_pps').append('<div><div role="alert" class="alert"><em class="fa fa-arrow-circle-left text-muted"></em>&nbsp;<center>No data available!!!<center></div></div>');
				return false;
			}
			else
			{

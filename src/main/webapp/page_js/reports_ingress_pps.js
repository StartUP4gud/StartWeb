...
function getData() {
	 var current_time = $('#input-calender-user').val();
	  ...
	  getReports(start, end, 'IN_PKTS');
	  getTableReports(start, end, 'IN_PKTS');
}
...
function getReports(startTime, endTime, type) {
	...
	$.ajax({
		...
		success: function(data){
			if(data.length==0){
				$('#chart_ingress_pps').append('<div><div role="alert" class="alert"><em class="fa fa-arrow-circle-left text-muted"></em>&nbsp;No data available!!!</div></div>');
				return false;
			}
			else
			{

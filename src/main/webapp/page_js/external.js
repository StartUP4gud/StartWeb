...
function getData() {
	...
	getReports(start, end, 'external');//DROP_PKTS
	getTableReports(start, end, 'external');
}
...
function getReports(startTime, endTime, type) {
	...
	$.ajax({
		...
		success: function(data){
			if(data.length==0){
				$('#chart_egress_bps').append('<div><div role="alert" class="alert"><em class="fa fa-arrow-circle-left text-muted"></em>&nbsp;<center>No data available!!!</center></div></div>');
				return false;
			}
			else
			{

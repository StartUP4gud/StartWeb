...
function getData() {
    ...
    getReports(start, end, 'Top_Application'); 
    getTableReports(start, end, 'Top_Application');
}
...
function getReports(startTime, endTime, type) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length == 0) {
                $('#chart_egress_fps').append('<div><div role="alert" class="alert"><em class="fa fa-arrow-circle-left text-muted"></em>&nbsp;<center>No data available!!!</center></div></div>');
                return false;
            } else {
                drawGraph(data);

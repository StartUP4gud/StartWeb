...
function getData() {
	...
	getReports(start, end, 'Top_country');
	getTableReports(start, end, 'Top_country');
}

function getReports(startTime, endTime, type) {

	$('#chart_ingress_bps').empty();
	...
	$.ajax({
		...
		success: function(data){
			if(data.length==0){
				$('#chart_ingress_bps').append('<div><div role="alert" class="alert"><em class="fa fa-arrow-circle-left text-muted"></em>&nbsp;<center>No data available!!!</center></div></div>');
				return false;
			}
			else
			{

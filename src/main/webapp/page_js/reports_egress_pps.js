...
function getData() {
	 var current_time = $('#input-calender-user').val();
	  ...
	  getReports(start, end, 'OUT_PKTS');
	  getTableReports(start, end, 'OUT_PKTS');
}

function getReports(startTime, endTime, type) {

	$('#chart_egress_pps').empty();
	...
	$.ajax({
		...
		success: function(data){
			if(data.length==0){
				$('#chart_egress_pps').append('<div><div role="alert" class="alert"><em class="fa fa-arrow-circle-left text-muted"></em>&nbsp;No data available!!!</div></div>');
				return false;
			}
			else
			{
